...
window.GPEN_COURSES = [
  /* ------------------------------------------------------------------ DASH II */
  {
    ...
    quiz: [
      ...
      { q: "\"I'll just roll it,\" the customer says, waving off the vape. What is the strongest next line?", choices: ["\"Rolling's fine if you're into it, though the vape is really just the more modern way to do it now.\"", "\"Rolling burns off the flavor, and some of it just goes up in smoke — this heats the same gram, so it goes further.\"", "\"Joints are honestly a bad habit — most people who switch over never go back to rolling papers again.\"", "\"Sure, but the vape is a lot more discreet — nobody standing around you knows what you're using.\""], answer: 1, why: "Arguing against a habit the customer already enjoys puts them on the defensive. Reframe instead, and anchor on waste and flavor — things they can judge for themselves, and the only ground the brand can stand on." },
      { q: "The customer is interested but hesitates: \"Is $49.95 really worth it?\" Besides holding the price, what should you do before they leave?", choices: ["Tell them the warranty covers them for a full year", "Offer to knock a few bucks off if they buy today", "Attach a USB-C cable so they can charge it", "Show them the $99.95 Dash+ so fifty feels cheap"], answer: 2, why: "Holding a price only wins the sale if the customer can actually use the thing that night — the Dash II box has no charging cable, so the sale is not finished until you close that gap. Discounting teaches them to wait for a deal, and the warranty is only a full year if they register." },
      { q: "Mid-pitch, a customer says smoking makes them cough and asks whether this device would help with that. Your move?", choices: ["Explain that vapor leaves behind the harsher parts of smoke", "Tell them heating instead of burning is gentler on the throat", "Say it depends on temperature and suggest they run it low", "Decline the health question and steer back to flavor and taste"], answer: 3, why: "Any comfort, health, or symptom claim becomes the brand's liability, not just yours. The safe and still-persuasive ground is flavor and experience — or telling them to ask a doctor." },
      ...
      { q: "A customer returns a few weeks after buying: \"It barely produces anything anymore.\" What is the first thing to check?", choices: ["Whether spent material has built up in the chamber", "Whether they are running the temperature too low", "Whether they are packing the chamber too tightly", "Whether the battery has stopped holding a charge"], answer: 0, why: "Check maintenance before hardware — residue in the ceramic chamber restricts airflow long before a battery or heater actually fails. Packing density matters too, which is why the Dash II is meant to be packed lightly rather than crammed." },
      { q: "Someone at the counter is buying a gram of wax and a 510 cartridge. They ask if this device handles both. What do you tell them?", choices: ["Both are fine as long as you load them with the pick tool", "The wax goes in the chamber, but the cart needs a battery", "The cart threads right on, but the wax has to go elsewhere", "Dry herb only, so neither of those two goes in it"], answer: 3, why: "Selling a device into the wrong material is the fastest way to turn a sale into a return. Name the limit first, then route them — wax to the Melt, the cart to a 510 battery — and you keep the customer instead of the refund." },
      { q: "A customer tries your floor demo, says the first pull tasted like nothing, and mentions they packed it in as firmly as they could and the draw felt tight. What do you correct?", choices: ["They were holding the button down through the whole draw", "They only filled the ceramic chamber about halfway", "They packed the chamber too tightly for air to pull through", "They should have set a higher temperature before starting"], answer: 2, why: "A dry-herb oven needs air moving across the load — cram it and heat never reaches the herb. Under-filling is wrong too (the spec is a full chamber, packed light), but that thins the vapor rather than killing the draw." },
      { q: "\"I've never used a dry-herb vape before\" — a hesitant first-timer. What framing closes them?", choices: ["Steer them to the pricier one since it is more forgiving", "Walk them through the settings menu so nothing is a surprise", "Frame it as the hard-to-mess-up one and keep it to a few steps", "Compare the temperature range against the higher-end model"], answer: 2, why: "Inexperience objections are fear of complexity, not a gap in information, so shrink the process to its four steps (grind, pack, hold to power on, double-tap to heat) instead of adding to it. Anything that piles on detail or price, like a settings tour or a step up in model, makes a nervous buyer more nervous." },
      { q: "A flower customer is clearly deciding between two dry-herb units. How should you present the choice?", choices: ["Start at $49.95 and only mention the other if they object", "Ask their budget first, then show only the tier that fits it", "Lead with the Dash+ — it is the better vaporizer of the two", "Ask whether they want the pocket Dash II or the titanium Dash+"], answer: 3, why: "A which-close beats a yes-or-no close because either answer is a sale. Framing the two units as a preference rather than a budget question also keeps the customer from feeling sold up or priced out." },
      { q: "\"Do I just drop a nug in?\" the customer asks, holding the open chamber. What do you say?", choices: ["Grind it first, then fill the chamber and pack it lightly", "A whole bud is fine as long as it fits the chamber", "Break it up by hand, since grinding makes it pack too tight", "Grind it and press it in firmly so it makes full contact"], answer: 0, why: "The load spec is three things at once: ground herb, filling the chamber, packed light with the pick tool. Each wrong instinct misses one of them, and it is the natural moment to attach a grinder to the sale." },
      { q: "A customer puts a half-ounce of flower on the counter and nothing else. Which two things attach to that sale?", choices: ["A Hydout and a 510 Original — everybody needs a battery", "A 3-Piece Grinder at $19.95 and a Dash II at $49.95 — they are already breaking it up, and the vaporizer is the next piece", "A Melt Hot Knife, so they are ready when they try concentrate", "Nothing — flower customers already have their setup"], answer: 1, why: "Attach to what is actually in the basket. Flower means grinding and heating, so the grinder and the Dash II are the two pieces that belong to this sale. A 510 battery attaches to a cartridge and a hot knife to concentrate — offering either here is guessing at a purchase they have not made, and it reads that way to the customer." },
      { q: "A flower customer picks up the Dash II and asks, \"so is this basically like smoking it?\"", choices: ["Pretty much, but it is smoother on your throat", "It is the healthier way to do it", "It heats the flower instead of burning it, so you taste the strain rather than smoke", "Yes — same thing, just electric"], answer: 2, why: "Heating versus burning is the whole product in one sentence, and it is a fact you can state. The two answers that reach for throat comfort or health are claims the brand cannot make and you should never make at the counter — and \"same thing, just electric\" throws away the only reason to buy it." }
    ],
  },
  ...
  /* ------------------------------------------------------------------- DASH+ */
  {
    ...
    howToSell: {
      ...
      scenarios: [
        ...
        { sees: "Buying a nice strain as a gift, or trading up from a Dash II", say: "For a gift or a step up, this is the one. Color display, a vibration when it reaches temperature, and a full titanium chamber. $99.95." },
      ],
    ...
    quiz: [
      ...
      { q: "\"Why is this double the price of the Dash II?\" What answers that best at the counter?", choices: ["It is the newer model, and the older one is being phased out", "Titanium chamber and hybrid convection, so you actually taste the strain", "It also takes concentrates, so one device covers your flower and your wax", "It has a bigger oven, so you fit more flower in each session"], answer: 1, why: "Price objections close on the mechanism behind the experience, not a feature list — name the oven and the heat, then let flavor justify the jump. Two things you cannot promise: a bigger chamber (the Dash+ oven is smaller than the Dash II's) and concentrate compatibility (it is dry herb only)." },
      { q: "A rep tells a Dash II owner to trade up because \"the Dash+ oven is bigger.\" What is wrong with that pitch?", choices: ["Both ovens hold the same, so lead with the color screen", "Nothing is wrong, the Dash+ does hold more herb per session", "The Dash+ chamber is smaller, the upgrade is heat quality", "The chamber claim is fine, but the real upgrade is the battery"], answer: 2, why: "The Dash+ chamber is about 0.3g against the Dash II's 0.4g. Promising more capacity sets up a disappointed customer and a return, so position it as the step up in heat, not size." },
      { q: "A customer comes back saying their new Dash+ barely produces anything. What do you walk them through first?", choices: ["Pack the chamber as tight as it will go and take short quick puffs", "Run it at the lowest temperature so one load stretches the whole session", "Top the battery off first, since a low charge weakens the heater", "Turn the temperature up, let it finish heating, then draw slowly and steadily"], answer: 3, why: "Weak vapor is usually technique, not a defect. Convection only works once the oven is fully up to temp and air is actually moving through it, so correct heat and draw before you look at anything else." },
      { q: "A daily user says their Dash+ has \"stopped hitting\" after a few months. What do you have them do first?", choices: ["Send it in under warranty, since battery output fades as cells age", "Let it cool, then swab the chamber and mouthpiece with isopropyl alcohol", "Rinse the chamber and mouthpiece under warm water, then dry it overnight", "Swab the chamber with isopropyl while it is still hot from a session"], answer: 1, why: "Weak vapor on a well-used dry herb vape is almost always resin in the chamber and air path, not a failed part. Alcohol goes on a cool device, and it has to air dry fully before it goes back together." },
      { q: "A shopper says smoking makes her cough and asks whether the Dash+ will be easier on her lungs. What do you say?", choices: ["Say you cannot speak to health effects, then talk about flavor instead", "Explain that vaporizing leaves the harmful part of the smoke behind", "Confirm it is the healthier option and suggest she ask a doctor too", "Tell her it is gentler on the lungs because it heats instead of burning"], answer: 0, why: "Health claims are not ours to make, and hedging one still counts as making it. Decline the medical question outright and move the conversation to flavor and experience." },
      { q: "Holding a gram of shatter, a customer asks if the Dash+ can handle it. What is the move?", choices: ["No, but the Hydout handles concentrate through a 510 cart", "Yes, on the lowest setting so it does not scorch", "Yes, once the silicone insert is seated in the chamber", "No, it is dry herb only, so pair it with the Melt"], answer: 3, why: "The Dash+ chamber takes ground flower only, so the answer is no at any temperature setting. Build the habit of matching material to device — flower to a Dash, concentrate to the Melt — so a no on one product becomes a yes on another." },
      { q: "You are ringing up a Dash+ and a Dash II on the same order. Which add-on question actually matters?", choices: ["Both need a cable, because neither box includes one at all", "Neither one needs a cable, both boxes ship with USB-C included", "The Dash II needs a USB-C cable, the Dash+ already includes one", "The Dash+ needs a USB-C cable, the Dash II already includes one"], answer: 2, why: "The Dash+ box includes a USB-C cable; the Dash II box does not. Check in-box contents before the customer leaves, because a device they cannot charge at home comes back as a complaint, not a repeat visit." },
      { q: "A customer is weighing the Dash II against the Dash+ and asks how long each one is covered for. What do you tell them?", choices: ["Both carry two years of coverage, but only the Dash+ can be extended by registering it", "Dash+ is one year, two with registration; the Dash II has no way to extend its coverage", "Dash+ is two years, three with registration; Dash II is six months, a year with registration", "Both are six months out of the box, and both extend to a full year once the customer registers"], answer: 2, why: "Coverage scales with the tier and is part of what the price jump actually buys, so quote it per product rather than as one blanket number for the line. Always name the registration step too, since the extension only exists if the customer does it." },
      { q: "A customer asks whether one charge on the Dash+ will get him through a long day out. What do you tell him?", choices: ["About three hours of heating per charge, so a full day out is covered", "About ten minutes of heating per charge, so he should carry a power bank", "Effectively unlimited if he keeps it plugged in, since it charges while in use", "About forty minutes of heating per charge, and it refills fast over USB-C"], answer: 3, why: "Answer battery questions in heating time, not vague all-day language — heating minutes are what the customer actually burns through. Pair the real number with the fast USB-C recharge so the expectation you set is one the device can keep." },
      { q: "Which opener follows the Dash+ playbook with a browser who has not touched one yet?", choices: ["If you care how your flower tastes, this is it, ready in twenty seconds", "This is our premium model, so it is the best one we have on the wall", "The zinc-alloy body and full-color screen are what set this one apart", "It runs a 1,800mAh cell with hybrid convection and a titanium oven"], answer: 0, why: "Nobody buys titanium; they buy taste. Lead with the outcome the customer can imagine, and keep the specs in reserve to back it up if they ask." },
      { q: "A regular who buys top-shelf flower every week already owns a grinder. What do you put in front of them?", choices: ["The Dash+ at $99.95 — the model that does that flower justice", "A second grinder in a different colour", "The Dash II at $49.95, because cheaper always closes faster", "The Melt Hot Knife — a step up in the range"], answer: 0, why: "Match the device to what they already spend on flower. Someone buying top-shelf weekly is telling you they care about taste, which is the Dash+'s argument. Defaulting to the cheaper model on that customer leaves the better fit unmentioned, and the Melt is a concentrate tool — it is not a step up from a dry herb vaporizer, it is a different category." },
      { q: "A customer turns the Dash+ over and says, \"this looks complicated.\" What answers that best?", choices: ["It is simpler than it looks once you read the manual", "It is the same as the Dash II, just more expensive", "You will get used to the temperature settings after a few sessions", "It tells you when it is ready — the display shows the temperature and it vibrates when it has reached it"], answer: 3, why: "Answer the fear, not the feature list. What worries them is not knowing when to inhale, and the device solves exactly that: it shows the temperature and buzzes when it gets there. Sending them to a manual confirms the fear, and \"you will get used to it\" asks them to buy a problem." }
    ],
  },
  ...
  /* ------------------------------------------------------- 3-PIECE GRINDER */
  {
    ...
    quiz: [
      ...
      { q: "Someone is buying flower and a Dash II. What do you add, and why?", choices: ["A 510 Original, so they have a battery for later", "Nothing — do not overload a sale that size", "The 3-Piece Grinder at $19.95 — a dry herb vaporizer only performs as well as the grind", "A second Dash II as a spare"], answer: 2, why: "The grinder is not an unrelated add-on here, it is what makes the device they just bought work properly: an even, loose pack is what lets the chamber heat the flower evenly. Tying the $19.95 to the $49.95 they have already committed to is the easiest attach on the counter, and it protects their experience of the vaporizer." },
      { q: "\"Twenty bucks for a grinder? I have a free one at home.\" What do you lead with?", choices: ["It is machined aluminum with a magnetic lid — there is nothing in it to wear out and nothing to charge", "It is on sale this week, so it is basically fifteen", "The free ones break within a few months", "It catches your kief, so it pays for itself"], answer: 0, why: "On an accessory, durability is the value argument — say what the money buys. Discounting into the objection teaches them to wait for a sale. Claiming other grinders fail is a durability claim you cannot back up. And never reach for kief on this one: it has no screen, and that promise is the return." }
    ],
  },
  /* -------------------------------------------------------- MELT HOT KNIFE */
  {
    ...
    quiz: [
      ...
      { q: "A gram of live resin and a quartz banger are on the counter together. What attaches?", choices: ["A 3-Piece Grinder — they will need to break something up", "The Melt Hot Knife at $24.95 — they have the rig, this is how it gets loaded cleanly", "A Dash II, so they can do flower too", "A Hydout, since concentrate needs a battery"], answer: 1, why: "They have already solved how to heat it; what is unsolved is getting the concentrate off the tool and into the banger without dragging half of it back out. That is the one job this does. A grinder is for flower and a 510 battery is for cartridges — neither belongs to a rig sale." },
      { q: "A customer with a quartz banger asks, \"so with this I can ditch the torch?\"", choices: ["Yes — it heats electrically, so the torch is done", "Only if you are running low-temp dabs under 302°F", "You will still want the torch for glass, but not on an e-rig", "Not quite — this gets the dab out of the jar and into the banger cleanly. You heat the banger the same way you do now"], answer: 3, why: "This single sentence decides whether the Melt stays sold. The tip runs around 150°C, which is for handling concentrate, not for heating glass — it replaces the cold dab tool, not the heat source. Promise otherwise and it comes back to the counter within a week, and the customer stops trusting the rest of your recommendations too." }
    ],
  },
  ...
  /* ------------------------------------------------------------------ HYDOUT */
  {
    ...
    quiz: [
      ...
      { q: "A customer buying a 510 cartridge mentions they use it on breaks at work. Which battery do you put in front of them, and why?", choices: ["The 510 Original — cheapest is always the easy yes", "Either one, they are the same underneath", "The Hydout at $24.95 — the mouthpiece stores inside the body, so there is nothing sticking out of a pocket", "The Hydout, because the water filtration makes it smoother"], answer: 2, why: "They told you the constraint: discretion. The Hydout answers it directly, so lead with that rather than with price. And there is no water in this device at all — it is a 510 battery. Repeating that mistake at the counter is a claim about a feature the product does not have." },
      { q: "\"Why would I pay double for that when the 510 Original is right there?\"", choices: ["It is a stronger battery, so your cart lasts longer", "The cheap one tends to burn carts out over time", "It is only twelve dollars more, basically the same price", "Discretion, plus five voltage settings instead of three presets"], answer: 3, why: "Answer a price gap with what the extra money buys, never with the money itself — discounting the gap teaches them the product is not worth it. Both batteries are 400mAh, so runtime claims are not true, and knocking the cheaper one is a reliability claim you cannot support." }
    ],
  },
  ...
  /* ------------------------------------------------------------- 510 ORIGINAL */
  {
    ...
    quiz: [
      ...
      { q: "A first-time customer is buying a single 510 cartridge. What do you say?", choices: ["Do you need a battery for that?", "First cartridge? Then you will need a battery — this one is $12.95 and you just breathe in, no buttons", "Let me know if you want a battery as well", "You will want the Hydout, it is the better one"], answer: 1, why: "Assume the need instead of asking permission — a yes-or-no question invites a no, and both of the softer openings are exactly that. Naming the price and the simplicity in the same breath removes the two reasons a first-timer hesitates. Reaching straight for the more expensive battery on someone's first cartridge is the wrong fit, not an upsell." },
      { q: "A customer asks how the 510 Original works. What is the shortest true answer?", choices: ["Click it five times, then hold the button while you inhale", "Set your voltage first, then hold the button down", "Screw the cartridge on and breathe in — it fires on the draw, no button needed", "Double-click to preheat, then draw"], answer: 2, why: "Draw activation is the entire reason this is the beginner battery: it works before you have learned anything about it. The button and the three voltage presets exist for people who want them, but leading with either turns the simplest device on the wall into a device with a manual." }
    ],
  },
];
